...
const CalculatorScreen = ({ selectedCiphers }) => {
  ...
  // Calculate results when input text or selected ciphers change
    ...
    debounce((text, ciphers) => {
      if (text.trim()) {
        ...
        calculatedResults.forEach((result, index) => {
          // Only expand the first result by default
      ...
      <View style={styles.container}>
        ...
        {results.length > 0 ? (
          ...
          <View style={styles.emptyContainer}>
            {inputText.trim() ? (
              <Text style={styles.emptyText}>No ciphers selected. Please select at least one cipher.</Text>
            ) : (
